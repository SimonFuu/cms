@extends('backend.layouts.layouts')
@section('content')
    <div class="box">
        <div class="box-header">
            <h3 class="box-title">权限列表</h3>
            <div class="box-tools">
                <a href="{{ route('backend.system.actions.add') }}" class="btn btn-info btn-sm">添加</a>
            </div>
        </div>
        <!-- /.box-header -->
        <div class="box-body table-responsive">
            <table class="table table-hover actions-list">
                <thead>
                <tr>
                    <th width="50">#</th>
                    <th width="100">权限</th>
                    <th width="200">URIs</th>
                    <th width="100">权重</th>
                    <th width="200">描述</th>
                    <th width="100">操作</th>
                </tr>
                </thead>
                <tbody>
                @foreach($actions as $action)
                    @if($action -> parent_id != 0)
                        @php($urls = json_decode($action -> sub_uris, true))
                        @php($urls = (implode('<br/>', ($urls ? $urls : []))))
                    @else
                        @php($urls = $action -> menu_uri)
                    @endif
                    <tr class="actions-list-trs">
                        <td>{{ $action -> id }}</td>
                        <td>{{ $action -> name }}</td>
                        <td><div class="actions-list-urls">{!! $urls !!}</div></td>
                        <td>{{ $action -> weight }}</td>
                        <td>{{ $action -> desc }}</td>
                        <td>
                            <a class="btn btn-xs btn-primary" href="{{ route('backend.system.actions.edit', ['id' => $action -> id]) }}">编辑</a>
                            <a class="btn btn-xs btn-danger" href="{{ route('backend.system.actions.delete', ['id' => $action -> id]) }}">删除</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
            </table>
        </div>
        <!-- /.box-body -->
        <div class="box-footer clearfix">
            {{ $actions -> links() }}
        </div>
    </div>
@endsection